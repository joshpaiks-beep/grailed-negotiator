(function initScraper(globalScope) {
  ...
  function isSold() {
    // Look for the specific sold banner text
    const main = document.querySelector("main") || document.body;
    const allElements = main.querySelectorAll("p, span, div, h1, h2, h3");
    for (const el of allElements) {
      const text = normalizeWhitespace(el.textContent);
      // Exact match for Grailed's sold banner
      if (/this listing sold/i.test(text)) return true;
    }

    // Also check for "Sold Price" label
    for (const el of allElements) {
      const text = normalizeWhitespace(el.textContent);

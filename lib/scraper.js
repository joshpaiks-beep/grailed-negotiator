(function initScraper(globalScope) {
  ...
  // ---------------------------------------------------------------------------
  // Message submission flow (MESSAGE-FIRST approach)
  // ---------------------------------------------------------------------------

  /**
   * Find the "Message" button on a listing page (next to the "Offer" button).
   */
  function findMessageButton() {
    const buttons = Array.from(document.querySelectorAll("button"));

    // Look for button with exact text "Message"
    const messageBtn = buttons.find((btn) => {
      const text = normalizeWhitespace(btn.textContent);
      return /^message$/i.test(text);
    });
    if (messageBtn) return messageBtn;

    // Fallback: button with "message" in aria-label
    const ariaBtn = buttons.find((btn) => {
      const label = btn.getAttribute("aria-label") || "";
      return /message/i.test(label);
    });
    if (ariaBtn) return ariaBtn;

    // Fallback: link/button that says "Message Seller"
    const msgSellerBtn = buttons.find((btn) => {
      const text = normalizeWhitespace(btn.textContent);
      return /message seller/i.test(text);
    });
    if (msgSellerBtn) return msgSellerBtn;

    return null;
  }

  /**
   * Find the chat/message textarea after the message modal opens.
   */
  function findChatTextarea() {
    // Try common patterns for Grailed's message input
    return (
      document.querySelector("textarea[placeholder*='message' i]") ||
      document.querySelector("textarea[placeholder*='Message' i]") ||
      document.querySelector("textarea[name*='message' i]") ||
      document.querySelector("textarea[data-testid*='message' i]") ||
  ...
  /**
   * Find the send button in the chat/message modal.
   */
  function findChatSendButton() {
    ...
    // Send message
    const sendMsgBtn = buttons.find((btn) => {
      const text = normalizeWhitespace(btn.textContent);
      return /send message/i.test(text);
    });
    if (sendMsgBtn) return sendMsgBtn;

    // Arrow/send icon button (common in chat UIs)
    const ariaBtn = buttons.find((btn) => {
      const label = btn.getAttribute("aria-label") || "";
    ...
    // Submit type button inside a form near a textarea
    const textarea = findChatTextarea();
    if (textarea) {
      const form = textarea.closest("form");
      if (form) {
        const formSubmit = form.querySelector("button[type='submit'], button");
  ...
  /**
   * Submit a DM message to a seller on their listing page.
   * This clicks "Message", waits for the chat to open, types the message, and sends it.
   */
  async function submitMessage(messageText) {
    console.log("[GrailedScraper] submitMessage called:", messageText);

    // Step 1: Find and click the Message button
    const messageButton = findMessageButton();
    if (!messageButton) {
      console.log("[GrailedScraper] Message button not found in DOM");
      return { ok: false, error: "Message button not found on listing page." };
    }

    console.log("[GrailedScraper] Clicking Message button:", messageButton.textContent);
    messageButton.click();

    // Step 2: Wait for chat/modal to appear (up to 3s)
    let chatInput = null;
    for (let attempt = 0; attempt < 10; attempt++) {
      await wait(300);
      chatInput = findChatTextarea();
      if (chatInput) break;
      console.log("[GrailedScraper] Waiting for chat textarea... attempt", attempt + 1);
    }

    if (!chatInput) {
      console.log("[GrailedScraper] Chat textarea not found after waiting");
      return { ok: false, error: "Chat input not found after clicking Message button. Modal may not have opened." };
    }

    console.log("[GrailedScraper] Found chat textarea, typing message");
    await wait(200);

    // Step 3: Type the message using React-compatible input setting
    chatInput.focus();
    await wait(100);

    // Check if it's a contenteditable div (some chat UIs use this)
    if (chatInput.getAttribute("contenteditable") === "true") {
      chatInput.textContent = "";
      chatInput.textContent = messageText;
      chatInput.dispatchEvent(new Event("input", { bubbles: true }));
      chatInput.dispatchEvent(new Event("change", { bubbles: true }));
    } else {
      // Standard textarea — use React-compatible setter
      const nativeTextareaSetter = Object.getOwnPropertyDescriptor(
        window.HTMLTextAreaElement.prototype, "value"
      )?.set;

      if (nativeTextareaSetter) {
        nativeTextareaSetter.call(chatInput, messageText);
      } else {
    ...
    // Step 4: Submit — try Send button first, then Enter key
    const sendButton = findChatSendButton();
    if (sendButton && !sendButton.disabled) {
      console.log("[GrailedScraper] Clicking send button:", sendButton.textContent);
      sendButton.click();
    } else {
      console.log("[GrailedScraper] No send button found or disabled, pressing Enter");
    ...
    await wait(500);
    console.log("[GrailedScraper] Message submitted successfully");
    return { ok: true };
  }

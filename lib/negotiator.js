(function initNegotiator(globalScope) {
  ...
  function decideNextAction({ listing, config, negotiation, sellerCounter }) {
    ...
    if (!asking) {
      return { action: "wait", reason: "Listing price could not be read." };
    }
    ...
    if (listing.canMakeOffer === false && sellerCounter == null) {
      return { action: "wait", reason: "Listing is not currently offerable." };
    }

    if (!negotiation.lastOffer) {
      const openingOffer = computeOpeningOffer(listing, config);
      if (!openingOffer) {
        return { action: "wait", reason: "Opening offer could not be computed." };
      }
